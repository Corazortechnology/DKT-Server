...
export const getSpecialRequestById = async (req, res) => {
  try {
    const requestId = req.params.id;
    console.log(requestId)
    const specialRequest = await specialRequestModel.findById(requestId)
      .populate("donor")
      .exec();
    ...
    res.status(200).json({
      success: true,
      message: "Special request fetched successfully",
      data: specialRequest,
    });
  } catch (error) {
    console.error("Error fetching special request by ID:", error);
    res.status(500).json({
      success: false,
